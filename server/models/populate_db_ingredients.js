  ...
  for (let i = 333; i <= 616; i++) {
    ...
    if (data.ingredients !== null) {
      data.ingredients[0].strIngredient =
        data.ingredients[0].strIngredient.toLowerCase();
      ...
      if (priceList.length === 0) {
        ...
        data.ingredients[0].priceItem = Number(
          medianPrice[1].substring(1)
        ).toFixed(2);
        ...
        if (medianPrice.length < 5) {
          ...
          data.ingredients[0].priceUnit = Number(
            medianPrice[2].substring(1)
          ).toFixed(2);
          data.ingredients[0].priceQuantity = medianPrice[4];
        }
      }
